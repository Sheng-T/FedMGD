...
class SVHNDataset(BaseDataset):
    def __init__(self, opt, idx=None,dataType='train'):
        if dataType!='public' and dataType!='global':
            if idx is None:
                h5_name = "{}_SVHN.h5".format(dataType)
            else:
                h5_name = '{}_SVHN_unique_{:d}.h5'.format(dataType,idx)
        elif dataType == 'public':
            h5_name = "upload_0.05_SVHN_unique.h5"
        else:
    ...
    def __getitem__(self, index):
        ...
        A = self.label[index]
        B = self.image[index]
        if self.input_nc == 3 and len(B.shape) == 2:
            B = B[:, :, np.newaxis].repeat(3, axis=2)
        if len(B.shape) == 4:
            B = B.squeeze(0)
        B = Image.fromarray(np.uint8(B.transpose(1, 2, 0)))
