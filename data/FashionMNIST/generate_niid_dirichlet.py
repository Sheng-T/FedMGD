...
def main():
    ...
    parser.add_argument("--sampling_ratio", type=float, default=0.4, help="Ratio for sampling training samples.")
    ...
    def process_user_data( data, n_sample, SRC_CLASSES, Labels=None, unknown_test=0, public_test=None):
        ...
        if args.format == "h5":
            ...
            for i in range(NUM_USERS):
                train_files.append(
                    h5py.File(os.path.join(args.save_dir, 'train_{}_unique_{:d}.h5'.format('fashionMNIST', i)), 'w'))
                test_files.append(
                    h5py.File(os.path.join(args.save_dir, 'test_{}_unique_{:d}.h5'.format('fashionMNIST', i)), 'w'))
