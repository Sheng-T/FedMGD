...
def get_dataset():
    train_dataset = CIFAR10(root='/home/liuyuan/st_codes/dataset/cifar10/', train=True , download=True)
    test_dataset = CIFAR10(root='/home/liuyuan/st_codes/dataset/cifar10/', train=False, download=True)

    test_mnist_random = {'data': test_dataset.data, 'targets': test_dataset.targets}
    state = np.random.get_state()
    np.random.shuffle(test_mnist_random['data'])
    np.random.set_state(state)
    np.random.shuffle(test_mnist_random['targets'])

    public_test_len = math.ceil(len(test_dataset.targets) * 0.1)
    ...
    dataset = {
        ...
        'targets': np.concatenate((train_dataset.targets, test_mnist_random['targets'][public_test_len:]), axis=0)}
    ...
    def process_user_data( data, n_sample, SRC_CLASSES, Labels=None, unknown_test=0, public_test=None):
        ...
        if args.format == "h5":
            ...
            for i in range(NUM_USERS):
                train_files.append(
                    h5py.File(os.path.join(args.save_dir, 'train_{}_unique_{:d}.h5'.format('cifar10', i)), 'w'))
                test_files.append(
                    h5py.File(os.path.join(args.save_dir, 'test_{}_unique_{:d}.h5'.format('cifar10', i)), 'w'))
            ...
            for idx in indices:
                public_file.create_dataset('images/MNIST_{:d}'.format(idx), data=public_test['data'][idx])
                public_file.create_dataset('labels/MNIST_{:d}'.format(idx),data=public_test['targets'][idx])
