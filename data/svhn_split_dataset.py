...
from data.svhn_dataset import SVHNDataset

class SVHNSplitDataset(BaseDataset):
    def __init__(self, opt,dataType= 'train'):
        ...
        if dataType!='public' and dataType!='global':
            for i in range(opt.n_client):
                self.split_db.append(SVHNDataset(opt, i,dataType))
            self.compute_weights()
        else:
            self.split_db.append(SVHNDataset(opt, 0, dataType))
            self.weights = []
            self.max_num = None
    ...
    def __getitem__(self, index):
        ...
        if self.dataType =='train':
            ...
            result = {'weights': self.weights,'max_num': self.max_num,'label_of_client': self.label_of_client,'fedlc_weights': self.class2data_}
        else:
