...
from data.emnist_dataset import EMNISTDataset

class EMNISTSplitDataset(BaseDataset):
    def __init__(self, opt,dataType= 'train'):
        ...
        if dataType!='public' and dataType!='global':
            for i in range(opt.n_client):
                self.split_db.append(EMNISTDataset(opt, i,dataType))
            self.compute_weights()
        else:
            self.split_db.append(EMNISTDataset(opt, 0, dataType))
            self.weights = []
            self.max_num = None
    ...
    def compute_weights(self):
        ...
        self.label_weight = np.zeros((self.opt.n_client, 26))
        self.label_of_client = np.ones((self.opt.n_client, 26))
        num_of_labels_uagan = np.zeros((self.opt.n_client, 26))
        self.class2data_ = []
        for i in range(self.opt.n_client):
            database = self.split_db[i]
            for k in range(26):
