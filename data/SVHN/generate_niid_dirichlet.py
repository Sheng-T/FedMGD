...
def get_dataset(split='letters'):
    ...
    test_dataset = SVHN(root='./', split='test', download=True, transform=transform)
    extra_dataset = SVHN(root='./', split='extra', download=True, transform=transform)

    n_sample = len(train_dataset.data)
    SRC_N_CLASS = 10
    test_mnist_random = {'data': test_dataset.data, 'targets': test_dataset.labels}
    state = np.random.get_state()
    np.random.shuffle(test_mnist_random['data'])
    np.random.set_state(state)
    np.random.shuffle(test_mnist_random['targets'])

    public_test_len = math.ceil(test_dataset.labels.shape[0] * 0.1)
    public_test = {'data': test_mnist_random['data'][:public_test_len],'targets': test_mnist_random['targets'][:public_test_len]}

    dataset = {
        'data': np.concatenate((train_dataset.data, test_mnist_random['data'][public_test_len:],extra_dataset.data), axis=0),
        'targets': np.concatenate((train_dataset.labels, test_mnist_random['targets'][public_test_len:],extra_dataset.labels), axis=0)}
...
def main():
    ...
    parser.add_argument("--name", type=str, default='SVHN', help="number of classification labels")
    ...
    def process_user_data( data, n_sample, SRC_CLASSES,dataset_name, Labels=None, unknown_test=0, public_test=None):
        ...
        if args.format == "h5":
            ...
            for i in range(NUM_USERS):
                train_files.append(
                    h5py.File(os.path.join(args.save_dir, 'train_{}_unique_{:d}.h5'.format(dataset_name, i)), 'w'))
                test_files.append(
                    h5py.File(os.path.join(args.save_dir, 'test_{}_unique_{:d}.h5'.format(dataset_name, i)), 'w'))
    ...
    process_user_data( data_by_class, n_sample, SRC_CLASSES,args.name, public_test=public_test)
