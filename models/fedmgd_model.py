...
class FedMGDModel(BaseModel):
    ...
    def backward_G(self):
        ...
        self.loss_G_GAN_all = self.adversarial_loss(pred_fake_upload, True) + self.auxiliary_loss(aux_fake_upload, self.rand_label)
    ...
    def sever_train(self,epoch):
        ...
        self.aggregate(epoch)

        for i in range(self.opt.num_epochs*10):

            noise = torch.randn(self.opt.gen_batch, self.opt.nz, 1, 1).to(self.device)
            label_origin = torch.randint(self.opt.n_class, (1, self.opt.gen_batch)).squeeze().to(self.device)
            ...
            if len(screen_label_) > 1:
                ...
                pred = self.netS(screen_img.detach())
                loss = self.auxiliary_loss(pred, screen_label)
                loss.backward()
        ...
        for i in range(len(self.real_A)):
            self.netC[i].load_state_dict(state)
