...
class FEDDFWithLabelModel(BaseModel):
    ...
    def aggregate(self, epoch,public_dataset):
        solns = []
        for i in range(len(self.real_A)):
        ...
        self.distillation(epoch,public_dataset)
    ...
    def distillation(self,epoch,public_dataset):
        logist_aggregate = None

        kdata = next(iter(public_dataset))

        label = kdata['A_0'].to(self.device)
        data = kdata['B_0'].to(self.device)
        ...
        self.train_S(data,label,logist_aggregate)
